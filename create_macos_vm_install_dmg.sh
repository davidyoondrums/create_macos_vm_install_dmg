...
installer_mounted_volume_mount="$(echo $installer_mounted_volume | cut -d / -f 1)"
ls /Volumes | grep -i "$installer_mounted_volume_mount" | while read ejectimage
  do
    sudo hdiutil detach "/Volumes/$ejectimage" -force -quiet
  done
